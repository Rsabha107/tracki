...
function refreshTaskSubtask(val) {
    var g_response;
    // alert('refreshTaskNotes')
    $.ajax({
...
$(document).ready(function () {
    ...
    $(".form-submit-task-new-subtask").submit(function (event) {
        ...
        var taskID = $("#subtask_parent_task_id_overview").val();
            ...
            $.ajax({
                ...
                success: function (result) {
                    if (!result["error"]) {
                        ...
                        $("#taskTabSub")
                            .empty("")
                            .append(refreshTaskSubtask(taskID));
    ...
    $(".form-submit-task-new-note").submit(function (event) {
        ...
        var button_text =
            btn_html != "" || btn_html != "undefined" ? btn_html : btn_val;
        var taskID = $("#note_parent_task_id_overview").val();
            ...
            $.ajax({
                ...
                success: function (result) {
                    if (!result["error"]) {
                        ...
                        $("#taskTabNotes")
                            .empty("")
                            .append(refreshTaskNotes(taskID));
                    } else {
                        submit_btn.html(button_text);
                        submit_btn.attr("disabled", false);
                        toastr.error(result["message"]);
    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            $.each(response.data.data, function (index, value) {
                ...
                html += '<a href="javascript:void(0);" id="edit_task" data-action="update" data-dismiss="modal" data-source="list" '+
                        'data-type="edit" data-table="task_table" data-redirect="list" data-id='+value.id+' role="button" title="add">'+
...
function actions2Formatter(value, row, index) {
    ...
    html =
        ...
        label_view +
        '">' +
        '<i class="fa-solid far fa-lightbulb text-body"></i></a>';
