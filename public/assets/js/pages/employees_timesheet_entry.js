...
$(document).ready(function () {
    ...
    $("body").on("click", "#edit_employee_timesheet_entry", function () {
        ...
        $.ajax({
            url: "/tracki/employee/timesheet/entries/get/" + id,
            method: "GET",
            async: true,
    ...
    $("body").on("click", "#deleteTimesheetEntry", function (e) {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.ajax({
                    url: "/tracki/employee/timesheet/entry/delete/" + id,
                    method: "GET",
...
function actions2Formatter(value, row, index) {
    console.log("employees_timesheet_entry.js inside actions2Formatter");
    html = "";
    html =
        html +
    ...
    if (can_edit) {
        ...
        html =
            html +
            // '<div class="font-sans-serif btn-reveal-trigger position-static">' +
            '<a href="javascript:void(0)" class="btn btn-sm" id="edit_employee_timesheet_entry" data-action="update" " data-type="edit" data-id="' +
            row.id +
            '" data-table="employee_timesheet_entry_table" data-bs-toggle="tooltip" data-bs-placement="right" title="' +
            label_update +
            '">' +
            '<i class="fa-solid fa-pen-to-square text-primary"></i></a>';
    ...
    if (can_delete) {
        html =
            html +
            '<a href="javascript:void(0)" class="btn btn-sm" data-table="employee_timesheet_entry_table" data-id="' +
            row.id +
            '" id="deleteEmployeetimesheet_entry" data-bs-toggle="tooltip" data-bs-placement="right" title="' +
            label_delete +
            '">' +
