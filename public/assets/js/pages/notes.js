$(document).ready(function () {
        ...
        $.ajax({
            ...
            success: function (result) {
                if (!result["error"]) {
                    ...
                    html += "        </div>";
                    html += "    </div>";
                    html += '    <div class="col col-auto flex-1">';
                    html +=
    ...
    // delete task item
    $("body").on("click", "#deleteTaskNote", function (e) {
        var id = $(this).data("id");
        var tableID = $(this).data("table");
        e.preventDefault();
        // alert("deleteTaskNote");
        // alert(tableID)
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.ajax({
                    url: "/tracki/task/note/" + id + "/delete",
                    type: "DELETE",
                    headers: {
...
function actionsFormatterNotes(value, row, index) {
    return [
           '<button title="' +
            label_delete +
            '" type="button" class="btn delete" data-id=' +
            row.id +
            ' data-table="task_note_table" data-type="tags" id="deleteTaskNote">' +
...
$("#task_status_filter,#tasks_person_filter,#tasks_project_filter","tasks_department_filter").on(
    "change",
    function (e) {
        e.preventDefault();
