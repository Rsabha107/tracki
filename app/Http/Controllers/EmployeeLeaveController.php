...
class EmployeeLeaveController extends Controller
{
    ...
    public function index()
    {
        ...
        return view('tracki.employee.leave.list', compact('employee_leaves', 'employees', 'employee_leave_types', 'employee_leave_statuses'));
    }
    ...
    public function list($id = null)
    {
        ...
        // LOG::info('inside EmployeeLeaveController::list:****************** ');
        // LOG::info(request()->all());
        ...
        if ($id) {
            $op = EmployeeLeave::where('employee_id', $id)->orderBy($sort, $order);
        } else {
            $op = EmployeeLeave::orderBy($sort, $order);
        }
        ...
        $op = $op->paginate(request("limit"))->through(function ($op) use ($user) {
            ...
            // if ($op->leave_statuses->title == 'Approved' && !$user->hasRole('manager')) {
            if ($user->hasRole('User')) {

                $href_disabled = ($op->leave_statuses->title == 'Pending') ? "" : "isDisabled";
            ...
            } elseif ($user->hasRole('SuperAdmin') || $user->hasRole('HRMSADMIN')) {
                $status = '<span class="badge badge-phoenix fs--2 badge-phoenix-' . $op->leave_statuses->color .
            ...
            $actions = $actions .
                '<a href="javascript:void(0)" class="' . $href_disabled . ' btn btn-sm" id="edit_employee_leave" data-type="edit" data-id=' .
                $op->id .
                ' data-table="employee_leave_table" data-bs-toggle="tooltip" data-bs-placement="right" title="Update">' .
                '<i class="fa-solid fa-pen-to-square text-primary"></i></a>' .
                '<a href="javascript:void(0)" class="' . $href_disabled . ' btn btn-sm" data-table="employee_leave_table" data-id="' .
                $op->id .
                '" id="deleteEmployeeLeave" data-bs-toggle="tooltip" data-bs-placement="right" title="Delete">' .
